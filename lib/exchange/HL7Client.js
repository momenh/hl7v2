...
class HL7Client extends EventEmitter {
  ...
  send(msg) {
    return new Promise(resolve => {
      if (typeof msg === 'string')
        msg = HL7Message.parse(msg, {customDict: this._customDict, ignoreParsingErrors: this._ignoreParsingErrors});
  ...
  _onHL7Data(data) {
    let msg;
    try {
      msg = HL7Message.parse(data, {encoding: this.encoding, customDict: this._customDict, ignoreParsingErrors: this._ignoreParsingErrors});
