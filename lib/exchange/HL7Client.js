...
class HL7Client extends EventEmitter {
   ...
   * @param {boolean} [param2.ignoreParsingErrors]
   * @param {boolean} [param2.encodeHl7DataTypes]
   */
  constructor(param1, param2) {
    ...
    if (options && options.ignoreParsingErrors)
      this._ignoreParsingErrors = options.ignoreParsingErrors;
    else
      this._ignoreParsingErrors = false;
    if (options && options.encodeHl7DataTypes === false)
      this._encodeHl7DataTypes = false;
    else
      this._encodeHl7DataTypes = true; //default is true
  ...
  send(msg) {
    return new Promise(resolve => {
      if (typeof msg === 'string')
        msg = HL7Message.parse(msg, {customDict: this._customDict, ignoreParsingErrors: this._ignoreParsingErrors, encodeHl7DataTypes: this._encodeHl7DataTypes});
      ...
      //if charset is not valid and ignore parsing errors flag is true then default to utf8
      if(this._ignoreParsingErrors && !iconv.encodingExists(msg.MSH[18].value))
        charset = 'utf8'
  ...
  _onHL7Data(data) {
    let msg;
    try {
      msg = HL7Message.parse(data, {encoding: this.encoding, customDict: this._customDict, ignoreParsingErrors: this._ignoreParsingErrors, encodeHl7DataTypes: this._encodeHl7DataTypes});
