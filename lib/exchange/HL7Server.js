...
class HL7Server extends EventEmitter {
  ...
  _onHl7Block(socket, block) {
    ...
    // Parse message
    try {
      msg = HL7Message.parse(block, {encoding: this.encoding, customDict: this._customDict, ignoreParsingErrors: this._ignoreParsingErrors, encodeHl7DataTypes: this._encodeHl7DataTypes});
      socket._errorCount = 0;
    } catch (e) {
