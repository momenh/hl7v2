...
class HL7FieldData {
   ...
   */
  constructor(field, def, level, customDict, ignoreParsingErrors) {
  ...
  defineComponent(sequence, def) {
    ...
    const comp = new HL7FieldData(this._field, def, 1, this._customDict, this._ignoreParsingErrors);
    ...
    for (const [i, v] of arr.entries()) {
      try {
        ...
        if(this._ignoreParsingErrors) {
          //ignore parsing errors by adding the value as text
