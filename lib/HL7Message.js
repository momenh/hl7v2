...
class HL7Message {
   ...
   */
  add(type) {
    const segment = new HL7Segment(this, {customDict: this._customDict, ignoreParsingErrors: this._ignoreParsingErrors});
    segment.parse(type);
  ...
  parse(buf, options) {
    ...
    for (const [i, line] of lines.entries()) {
      if (!line)
        continue;
      const segment = new HL7Segment(this, {customDict, ignoreParsingErrors: this._ignoreParsingErrors});
      try {
        segment.parse(line);
      } catch (e) {
